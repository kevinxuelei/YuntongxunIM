//
//  YTXConversationDataSource.h
//  YTXIMKit
//
//  Created by dongxuelei on 2020/7/6.
...
@class YTXConversationViewController,YTXMessageModel,YTXMessage,YTXConversationViewLayout,YTXConversation;

@interface YTXConversationDataSource : NSObject

- (instancetype)init:(YTXConversationViewController *)chatVC;
//是否所有消息都已经加载完成
@property (nonatomic, assign, readonly) BOOL allMessagesAreLoaded;
//collection 的 layout
@property (nonatomic, strong, readonly) YTXConversationViewLayout *customFlowLayout;
//显示查看未读的消息 id
...
//进入聊天页面初次加载的消息
- (void)getInitialMessage:(YTXConversation *)conversation;
...
//设置消息是否显示用户名称
- (YTXMessageModel *)setModelIsDisplayNickName:(YTXMessageModel *)model;
//往数据源中添加发送出去的消息
- (void)appendSendOutMessage:(YTXMessage *)message;

- (void)appendAndDisplayMessage:(YTXMessage *)rcMessage;
//撤回消息
- (void)didRecallMessage:(YTXMessage *)recalledMsg;
//刷新被撤回消息的 UI
- (void)didReloadRecalledMessage:(YTXMessageModel *)messageModel;
...
//处理聊天页面收到的消息
- (void)didReceiveMessageNotification:(YTXMessage *)rcMessage leftDic:(NSDictionary *)leftDic;
@end
