//
//  YTXConversationListViewController.h
//  YTXIMKit
//
...
@interface YTXConversationListViewController : YTXBaseViewController<UITableViewDataSource, UITableViewDelegate>
#pragma mark - 初始化

/*!
 初始化会话列表

 @param displayConversationTypeArray    列表中需要显示的会话类型数组(需要将YTXConversationType转为NSNumber构建Array)
 @param collectionConversationTypeArray
 列表中需要聚合为一条显示的会话类型数组(需要将YTXConversationType转为NSNumber构建Array)
 @return                                    会话列表对象
...
/*!
 ...
 @discussion 数组中的元素为YTXConversationType转换的NSNumber
 */
...
/*!
 ...
 @param conversationTypeArray 列表中需要显示的会话类型数组(需要将YTXConversationType转为NSNumber构建Array)
 */
...
/*!
 ...
 @param conversationTypeArray 列表中需要聚合为一条显示的会话类型数组(需要将YTXConversationType转为NSNumber构建Array)
 */
...
/*!
 ...
 @discussion 数据源中存放的元素为会话Cell的数据模型，即YTXConversationModel对象。
 ...
 */
- (void)onSelectedTableRow:(YTXConversationModelType)conversationModelType
         conversationModel:(YTXConversationModel *)model
...
/*!
 ...
 @param model   会话Cell的数据模型
 */
//- (void)didDeleteConversationCell:(YTXConversationModel *)model;
...
/*!
 ...
 @param dataSource      即将加载的增量数据源（元素为YTXConversationModel对象）
 @return                修改后的数据源（元素为YTXConversationModel对象）

 @discussion 您可以在回调中修改、添加、删除数据源的元素来定制显示的内容，会话列表会根据您返回的修改后的数据源进行显示。
 数据源中存放的元素为会话Cell的数据模型，即YTXConversationModel对象。
 ...
 */
//- (NSMutableArray<YTXConversationModel *> *)willReloadTableData:(NSMutableArray<YTXConversationModel *> *)dataSource;
 ...
 */
//- (void)willDisplayConversationTableCell:(YTXConversationBaseCell *)cell atIndexPath:(NSIndexPath *)indexPath;
 ...
 */
//- (YTXConversationBaseCell *)rcConversationListTableView:(UITableView *)tableView
...
/*!
 ...
 notification的object为YTXMessage消息对象，userInfo为NSDictionary对象，其中key值为@"left"，value为还剩余未接收的消息数的NSNumber对象。
 */
