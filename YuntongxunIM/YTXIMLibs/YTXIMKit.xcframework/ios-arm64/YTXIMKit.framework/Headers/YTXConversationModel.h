...
/*!
 会话Cell数据模型的显示类型
 */
typedef NS_ENUM(NSUInteger, YTXConversationModelType) {
    /*!
     默认显示
     */
...
/*!
 会话Cell的数据模型类
 */
@interface YTXConversationModel : NSObject

@property (nonatomic, strong) YTXConversation *model;

/*!
 会话Cell数据模型的显示类型
 */
@property (nonatomic, assign) YTXConversationModelType conversationModelType;
...
//当前会话是否是免打扰状态
//*/
//@property (nonatomic, assign) YTXConversationNotificationStatus blockStatus;
//
///*!
// 置顶Cell的背景颜色
// */
...
// 会话中最后一条消息的内容
// */
//@property (nonatomic, strong) YTXMessageContent *lastestMessage;
//
///*!
// 会话中最后一条消息的方向
// */
//@property (nonatomic, assign) YTXMessageDirection lastestMessageDirection;
...
// */
//- (instancetype)initWithConversation:(YTXConversation *)conversation extend:(id)extend;
//
///*!
// 更新数据模型中的消息
//
...
// */
//- (void)updateWithMessage:(YTXMessage *)message;
//
///*!
// 会话和数据模型是否匹配
//
...
// @return 会话和数据模型是否匹配
// */
//- (BOOL)isMatching:(YTXConversationType)conversationType targetId:(NSString *)targetId;
@end
